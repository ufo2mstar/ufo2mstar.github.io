...
const JSON_OPS = new Set(['json-prettify', 'json-deflate']);

/** @param {State} s @returns {{ output: string, outputLanguage: string }} */
function execute(s) {
  const input = s.input || '';
  try {
    let output;
    ...
    return { output, outputLanguage: JSON_OPS.has(s.op) ? 'json' : 'text' };
  } catch (e) {
